...
def simulate():
    try:
        ...
        # -------- MASS FLOW --------
        m_dot = float(min(m_dot_engine, T / ve)) if ve > 0 else 0.0
        
        # -------- FUEL --------
        fuel_used = m_dot * burn_time
        m_after = float(max(m_current - fuel_used, 0))
        ...
        velocity_curve = []
        sim_v = float(v_current)
        sim_m = float(m_current)
        m_dot_float = float(m_dot)
        thrust_float = float(T)
        drag_float = float(D)
        for t in range(0, 30):
            sim_a = (thrust_float - drag_float - (sim_m * float(g) * float(np.cos(gamma_angle)))) / sim_m
            sim_v += sim_a * 5.0
            sim_m -= m_dot_float * 5.0
            if sim_v < 0: sim_v = 0.0
            if sim_m < 0: sim_m = 0.0
            velocity_curve.append(round(sim_v, 2))

        # Format output
        response = {
            "performance": {
                "velocity": round(float(v_current), 2),
                "thrust": round(float(T) / 1000.0, 2), # kN
                "power": round(float(power) / 1000000.0, 2), # MW
                "dynamicPressure": round(float(q), 2), # Pa
                "drag": round(float(D) / 1000.0, 2) # kN
            },
            "fuel": {
                "fuelUsed": round(float(fuel_used), 2),
                "remainingMass": round(float(m_after), 2),
                "massFlowRate": round(float(m_dot), 2)
            },
            "engine": {
                "maxFlowRate": round(float(m_dot_engine), 2),
                "throttlePercentage": round(float(throttle) * 100.0, 1),
                "twr": round(float(TWR), 2)
            },
            "analysis": {
                "machStatus": str(mach_status),
                "qStatus": str(q_status),
                "fuelStatus": str(fuel_status),
                "twrCondition": str(twr_status),
                "efficiency": str(eff_status)
            },
